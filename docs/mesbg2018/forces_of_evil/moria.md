## ARMY COMPOSITION

This army may include: The Balrog; Dragon; Durburz; Ashrak; Druzhag; Groblog; Moria Blackshield Captain; Moria Goblin Captain; Cave Drake; Moria Blackshield Shaman; Moria Goblin Shaman; The Watcher in the Water; Bat Swarm; Cave Troll; Dweller in the Dark; Giant Spider; Moria Blackshield; Moria Blackshield Drummers; Moria Goblin Drum; Moria Goblin Prowler; Moria Goblin Warrior; Warg Marauder; Wild Warg.

## ARMY BONUS

**"We cannot get out, they are coming"** - Friendly Moria Goblin models involved in a Fight where their opponent is Trapped receive a bonus of +1 to their Fight value.

## HEROES OF MORIA

<div class="unitCard" markdown>

### THE BALROG [350 POINTS]
*HERO OF LEGEND, UNIQUE, 18 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 10/3+ | 9 | 9 | 4 | 10 | 7 | 0 | 10 | 0 |

**WARGEAR**

- Giant Flaming [Sword]
- Fiery Lash

**SPECIAL RULES**

- [Ancient Evil]
- [Fearless]
- [Resistant to Magic]
- [Terror]

- **Fiery Lash** `ACTIVE`

Range 8", S7, and pulls model into base contact with The Balrog.

- **Goblin Mastery** `PASSIVE`

Moria Goblins within 12" automatically pass Courage tests.

- **Flame of Udun** `PASSIVE`

If an enemy is Wounded, roll D6. On 6, the model is [Set Ablaze].

- **Demon of the Ancient World** `PASSIVE`

May call a free [Heroic Combat] each turn.

</div>

---

<div class="unitCard" markdown>

### DRAGON [250 POINTS]
*HERO OF VALOUR, 15 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 7/2+ | 7 | 7 | 4 | 7 | 4 | 3 | 3 | 3 |

**WARGEAR**

- Claws and Teeth

**HEROIC ACTIONS**

- [Heroic Channelling]
- [Heroic Strike]
- [Heroic Strength]
- [Heroic Defence]

**MAGICAL POWERS**

| Power | Range | Casting Value |
|:--|:--:|:--:|
| [Transfix] | 12" | 3+ |
| [Compel] | 12" | 4+ |

**OPTIONS**

- Breathe Fire - 50 points
- Fly - 50 points
- Tough Hide - 50 points
- Wyrmtongue - 50 points

**SPECIAL RULES**

- [Harbinger of Evil]
- [Monstrous Charge]
- [Resistant to Magic]
- [Survival Instinct]
- [Terror]

</div>

---

<div class="unitCard" markdown>

### DURBURZ [70 POINTS]
*HERO OF VALOUR, UNIQUE, 15 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 4/5+ | 4 | 6 | 2 | 2 | 4 | 3 | 2 | 2 |

**WARGEAR**

- [Heavy Armour]
- [Sword]

**HEROIC ACTIONS**

- [Heroic Strike]
- [Heroic Resolve]
- [Heroic Defence]

**SPECIAL RULES**

- [Cave Dweller]

- **Iron Fist** `ACTIVE`

Durburz's [Stand Fast!] has a range of 12"

</div>

---

<div class="unitCard" markdown>

### ASHRAK [55 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 4 | 1 | 2 | 4 | 1 | 4 | 1 |

**WARGEAR**

- [Armour]
- [Two-handed][Two-Handed] [Staff]

**HEROIC ACTIONS**

- [Heroic Channelling]

**MAGICAL POWERS**

| Power | Range | Casting Value |
|:--|:--:|:--:|
| [Fury] (Spider) | - | 3+ |
| [Shroud of Shadows] | 12" | 4+ |

**SPECIAL RULES**

- [Cave Dweller]

- **Death-touch** `PASSIVE`

Any model that wounds, or is wounded by Ahshrak must roll a D6. On a 4+, they suffer the effects of [Paralyse].

- **Poison Blood** `PASSIVE`

May include Giant Spiders in his warband. Giant Spiders in the same force can be upgraded to Venom-back spiders for 2 points, and gain [Venom] special rule.

- **Spider-kin** `PASSIVE`

Only Spider models may benefit from Ashrak's [Stand Fast!], Heroic Actions, and [Shroud of Shadows] magical power.

</div>

---

<div class="unitCard" markdown>

### DRUZHAG [90 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 3/5+ | 3 | 4 | 1 | 2 | 4 | 2 | 5 | 2 |

**WARGEAR**

- [Armour]
- [Dagger]
- [Staff]

**HEROIC ACTIONS**

- [Heroic Channelling]

**MAGICAL POWERS**

| Power | Range | Casting Value |
|:--|:--:|:--:|
| [Enrage Beast] | 12" | 3+ |
| [Fury] (Bat, Warg, Spider) | - | 3+ |

**SPECIAL RULES**

- [Cave Dweller]

- **Master of the Dark Wild** `PASSIVE`

Druzhag may include Wild Wargs, Fell Wargs, Giant Spiders and Bat Swarms in his warband. All friendly Bat, [Warg] and Spider models within 12" may use Druzhag's Courage value instead of their own. Additionally, his [Fury] power will affect himself and any model that is a Bat, [Warg] or Spider.

</div>

---

<div class="unitCard" markdown>

### GROBLOG [50 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 3/5+ | 4 | 6 | 2 | 2 | 3 | 3 | 1 | 1 |

**WARGEAR**

- [Armour]
- [Sword]
- Mithril Crown

**HEROIC ACTIONS**

- [Heroic Strike]
- [Heroic Defence]

**SPECIAL RULES**

- [Cave Dweller]

- **Mithril Crown** `PASSIVE`

At the start of the Fight phase, before Heroic Actions are declared, roll a D6. On a 4+, all Moria Goblins within 3" gain +1 Fight value for the rest of that turn.

</div>

---

<div class="unitCard" markdown>

### MORIA BLACKSHIELD CAPTAIN [45 POINTS]
*HERO OF FORTITUDE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 3/5+ | 4 | 6 | 2 | 2 | 4 | 2 | 1 | 1 |

**WARGEAR**

- [Heavy Armour]
- [Two-handed][Two-Handed] [Sword]

**HEROIC ACTIONS**

- [Heroic March]

**OPTIONS**

- Swap Weapon - 5 points

**SPECIAL RULES**

- [Cave Dweller]
- [Hatred] (Dwarf)

</div>

---

<div class="unitCard" markdown>

### MORIA GOBLIN CAPTAIN [35 POINTS]
*HERO OF FORTITUDE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 3/5+ | 4 | 5 | 2 | 2 | 3 | 2 | 1 | 1 |

**WARGEAR**

- [Armour]
- [Sword]

**HEROIC ACTIONS**

- [Heroic March]

**OPTIONS**

- Orc [Bow] - 5 points
- [Shield] - 5 points
- Swap Weapon - 5 points

**SPECIAL RULES**

- [Cave Dweller]

</div>

---

<div class="unitCard" markdown>

### CAVE DRAKE [150 POINTS]
*MINOR HERO, 6 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 8" | 6/5+ | 7 | 7 | 4 | 6 | 4 | 1 | 3 | 1 |

**WARGEAR**

- Claws and Teeth

**HEROIC ACTIONS**

- [Heroic Strength]

**SPECIAL RULES**

- [Monstrous Charge]
- [Mountain Dweller]
- [Resistant to Magic]
- [Terror]
- [Woodland Creature]

- **Protect the Nest** `ACTIVE`

At the start of the game, place a 25mm Nest marker. Whilst within 3" of the Nest, Cave Drake gains the [Fearless] special rule.

- **Gaping Maw** `BRUTAL POWER ATTACK`

Instead of striking as normal, select a single Man-sized (or smaller model) and roll a D6. On a 3+, the model is swallowed whole and slain. The enemy model can still use Fate to prevent the attack and not be slain.

</div>

---

<div class="unitCard" markdown>

### MORIA BLACKSHIELD SHAMAN [50 POINTS]
*MINOR HERO, 6 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 4 | 1 | 2 | 3 | 1 | 3 | 1 |

**WARGEAR**

- [Armour]
- [Staff]

**HEROIC ACTIONS**

- [Heroic Channelling]

**MAGICAL POWERS**

| Power | Range | Casting Value |
|:--|:--:|:--:|
| [Shatter] | 12" | 3+ |
| [Tremor] | 6" | 5+ |

**SPECIAL RULES**

- [Cave Dweller]

</div>

---

<div class="unitCard" markdown>

### MORIA GOBLIN SHAMAN [45 POINTS]
*MINOR HERO, 6 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 4 | 1 | 2 | 3 | 1 | 3 | 1 |

**WARGEAR**

- [Armour]
- [Spear]

**HEROIC ACTIONS**

- [Heroic Channelling]

**MAGICAL POWERS**

| Power | Range | Casting Value |
|:--|:--:|:--:|
| [Fury] (Moria Goblin) | - | 3+ |
| [Transfix] | 12" | 5+ |

**SPECIAL RULES**

- [Cave Dweller]

</div>

---

<div class="unitCard" markdown>

### THE WATCHER IN THE WATER [200 POINTS]
*INDEPENDENT HERO, UNIQUE*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 4" | 6/3+ | 6 | 6 | 6 | 3 | 3 | 1 | 5 | 1 |

**WARGEAR**

- Tentacles

**HEROIC ACTIONS**

- [Heroic Strength]

**SPECIAL RULES**

- [Harbinger of Evil]
- [Resistant to Magic]
- [Terror]

- **From the Deep** `ACTIVE`

See page 159 of Armies of LOTR book.

- **Many Tentacles** `PASSIVE`

At 3 Wounds or less, The Watcher's Attack value becomes 3.

- **Tentacles** `ACTIVE`

The Watcher can make D6 shooting attacks, range 6" and S3, with no In The Way tests. Models hit are dragged into base contact.

- **Water Dweller** `PASSIVE`

Whilst within water, The Watcher's Move distance is doubled, and gains the [Monstrous Charge] special rule.

- **Gaping Maw** `BRUTAL POWER ATTACK`

Whilst within water, The Watcher can choose a single Man-sized (or smaller) model and instantly slay them unless saved with Fate. The Watcher is removed from the board and needs to re-arrive as per the From the Deep special rule.

</div>

---

## WARRIORS OF MORIA

<div class="unitCard" markdown>

### BAT SWARM [35 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 3" | 1/5+ | 3 | 3 | 2 | 4 | 2 |

**WARGEAR**

- Claws and Teeth

**SPECIAL RULES**

- [Fly]

- **Blinding Swarm** `PASSIVE`

The Fight value of any enemy model engaged in combat with a Bat Swarm is halved (rounding down).

</div>

---

<div class="unitCard" markdown>

### CAVE TROLL [75 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 6/5+ | 6 | 6 | 3 | 3 | 3 |

**WARGEAR**

- [Spear]

**OPTIONS**

- Troll Chain - 5 points
- Swap [Spear] for [Hammer] - 5 points
- Swap Weapon - 1 point

**SPECIAL RULES**

- [Burly]
- [Terror]
- [Throw Stones] (range 12", Strength 8)

- **Troll Chain** `ACTIVE`

A [throwing weapon][Throwing Weapon] with range 3" and Strength 5.

</div>

---

<div class="unitCard" markdown>

### DWELLER IN THE DARK [75 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 8" | 7/3+ | 5 | 5 | 3 | 3 | 7 |

**WARGEAR**

- Claws and Teeth

**SPECIAL RULES**

- [Resistant to Magic]
- [Terror]

- **Murderous Power** `ACTIVE`

Whenever a Dweller in the Dark slays an enemy model in combat, it regains a single Wound lost earlier in the battle. Rend is the only Brutal Power Attack that this effect also applies to.

</div>

---

<div class="unitCard" markdown>

### GIANT SPIDER [20 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 10" | 4/6+ | 5 | 3 | 2 | 2 | 3 |

**WARGEAR**

- Venomous Fangs

**OPTIONS**

- Venom-back Spider - 2 points

**SPECIAL RULES**

- Poisoned Fangs
- [Swift Movement]
- [Terror]

</div>

---

<div class="unitCard" markdown>

### MORIA BLACKSHIELD [8 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 6 | 1 | 1 | 3 |

**WARGEAR**

- [Heavy Armour]
- [Sword]
- [Shield]

**OPTIONS**

- [Spear] - 1 point
- Swap Weapon - 1 point

**SPECIAL RULES**

- [Cave Dweller]
- [Hatred] (Dwarf)

</div>

---

<div class="unitCard" markdown>

### MORIA BLACKSHIELD DRUMMERS [75 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 5 | 1 | 1 | 3 |

**WARGEAR**

- [Heavy Armour]
- Clubs
- One Drum

**OPTIONS**

- Swap Weapon - 1 point

**SPECIAL RULES**

- [Cave Dweller]
- [Hatred] (Dwarf)

- **Take up the Drum** `PASSIVE`

If a Drummer or Bearer is slain, they may pass on their wargear to any unengaged Moria Blackshield within 1".

- **Run and Drum** `ACTIVE`

If Drummer and Bearer are in base contact, the drum is playing.

- **Doom! Doom!** `ACTIVE`

Drum effects found on page 161 of Armies of LOTR book.

</div>

---

<div class="unitCard" markdown>

### MORIA GOBLIN DRUM [75 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| - | - | - | 10 | - | 3 | - |

**OPTIONS**

- Swap Weapon - 1 point

**SPECIAL RULES**

- **Destroy the Drum** `PASSIVE`

Any model that spends a turn in base contact with a drum without doing anything else (such as shooting) may destroy the drum.

- **Take up the Drum** `PASSIVE`

If a Drummer is slain, they may pass on their wargear to any unengaged Moria Goblin Warrior within 1".

- **Drums in the Deep** `ACTIVE`

A Drummer must be in contact with the drum, and not Engaged. Drum effects found on page 161 of Armies of LOTR book.

</div>

---

<div class="unitCard" markdown>

### MORIA GOBLIN PROWLER [7 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 3/4+ | 3 | 4 | 1 | 1 | 2 |

**WARGEAR**

- [Armour]
- [Two-handed][Two-Handed] [Axe]
- Throwing Daggers

**OPTIONS**

- Swap Weapon - 1 point

**SPECIAL RULES**

- [Cave Dweller]
- [Backstabbers]

</div>

---

<div class="unitCard" markdown>

### MORIA GOBLIN WARRIOR [4 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 5" | 2/5+ | 3 | 4 | 1 | 1 | 2 |

**WARGEAR**

- [Armour]
- [Sword]

**OPTIONS**

- Orc [Bow] - 1 point
- [Shield] - 1 point
- [Spear] - 1 point
- Swap Weapon - 1 point

**SPECIAL RULES**

- [Cave Dweller]

</div>

---

<div class="unitCard" markdown>

### WARG MARAUDER [30 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 10" | 3/5+ | 4 | 4 | 1 | 1 | 2 |

**WARGEAR**

- Claws and Teeth

**OPTIONS**

- Swap Weapon - 1 point

**SPECIAL RULES**

- <span style="color: darkmagenta">[Cave Dweller]</span>

- **Lethal Union** `PASSIVE`

Whilst Goblins remain mounted, entire model gains [Terror], and +1 Courage. In combat, the model makes +1 Attack for each Goblin still mounted after the first, as long as they didn't shoot.

- **On the Hunt** `ACTIVE`

Goblins mounted with Orc bows can still fire even after the [Warg] Marauder has moved its full distance.

</div>

---

<div class="unitCard" markdown>

### WILD WARG [7 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 10" | 3/5+ | 4 | 4 | 1 | 1 | 2 |

**WARGEAR**

- Claws and Teeth

</div>
