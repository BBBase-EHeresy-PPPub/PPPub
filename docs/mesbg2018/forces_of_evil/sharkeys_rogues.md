## ARMY COMPOSITION

This army may include: Sharkey and Worm; Sid Briarthorn, Ruffian Chief; Bill Ferny; Harry Goatleaf; Lotho Sackville-Baggins; Rowan Thistlewood, Ruffian Enforcer; Ruffian; Ted Sandyman; Hobbit Militia; Ruffian.

## ARMY BONUS

**"You do what Sharkey says!"** - Sharkey's Stand Fast! affects the entire battlefield. Additionally, a force (or allied contingent) that contains models solely from the Sharkey's Rogues list may include warbands of 10-12 Ruffians without the need for a Hero to lead them. In these warbands, nominate a single Warrior model within the warband as the captain for the purposes of deployment, etc.


## HEROES OF SHARKEY'S ROGUES

<div class="unitCard" markdown>

### SHARKEY AND WORM [60 POINTS]
*HERO OF LEGEND, UNIQUE, 18 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| - | - | - | - | - | - | - | 1 | 4 | 1 |

</div>

---

<div class="unitCard" markdown>

### SID BRIARTHORN, RUFFIAN CHIEF [45 POINTS]
*HERO OF VALOUR, UNIQUE, 15 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 4/4+ | 4 | 4 | 2 | 2 | 4 | 2 | 1 | 1 |

**WARGEAR**

- Jagged knife (sword)
- club
- whip

**HEROIC ACTIONS**

- Heroic Strength

**SPECIAL RULES**

- Terror (Hobbit)

- **With Me Lads!** `PASSIVE`

Other friendly Ruffian models treat Sid as a banner.

- **Put 'em in Chains** `ACTIVE`

Instead of making Strikes, Sid may choose a Hobbit model and roll a D6. If higher than the Hobbit's Strength, the Hobbit is bound in chains, treated as if it had suffered the Paralyse Magic Power.

- **Brawler** `PASSIVE`

Never counts as being unarmed.


</div>

---

<div class="unitCard" markdown>

### BILL FERNY [40 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 3/4+ | 3 | 3 | 1 | 2 | 3 | 1 | 1 | 1 |

**WARGEAR**

- Dagger
- whip

**SPECIAL RULES**

- Hatred (Hobbit)

- **Sharkey's Spy** `PASSIVE`

At the start of the game, Bill may choose an enemy warband to spy on. In Scenarios where you roll for reinforcements to arrive, or where they will deploy, that warband suffers -1 on their roll.

- **Put 'em in Chains** `ACTIVE`

Instead of making Strikes, Sid may choose a Hobbit model and roll a D6. If higher than the Hobbit's Strength, the Hobbit is bound in chains, treated as if it had suffered the Paralyse Magic Power.


</div>

---

<div class="unitCard" markdown>

### HARRY GOATLEAF [40 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 3/4+ | 3 | 3 | 1 | 2 | 3 | 1 | 3 | 1 |

**WARGEAR**

- Dagger
- Lantern

**SPECIAL RULES**

- Hatred (Hobbit)

- **Lantern** `PASSIVE`

The area within 12" of Harry is illuminated. Models within this area can be seen clearly, unless another special rule would prevent them from doing so.

- **Gatekeeper** `ACTIVE`

After Priority has been rolled, Harry may spent a point of Will and choose one enemy Infantry model within 6" and in LoS of Harry, and roll a D6. On a 3+, that model cannot move that turn, but may perform any other action as normal.


</div>

---

<div class="unitCard" markdown>

### LOTHO SACKVILLE-BAGGINS [30 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 4" | 2/3+ | 2 | 3 | 1 | 1 | 3 | 1 | 4 | 1 |

**WARGEAR**

- Dagger

**HEROIC ACTIONS**

- Heroic Accuracy

**SPECIAL RULES**

- Resistant to Magic
- Throw Stones (range 8", Strength 1)

- **Considerable Wealth** `ACTIVE`

Lotho may spend his Will points to pay off models around him, in the following three ways:

1. If an enemy charges Lotho, roll a D6. On a 3+, the model stops 1" away and may not Charge.

2. All friendly Ruffian models within 3" will automatically pass their next Courage test.

3. All friendly Hobbit models within 3" add +1 to their Fight value until the end of that turn.

- **Family Rivalry** `PASSIVE`

Lotho cannot take part in any Heroic Actions from Bilbo or Frodo.


</div>

---

<div class="unitCard" markdown>

### ROWAN THISTLEWOOD, RUFFIAN ENFORCER [45 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 3/4+ | 4 | 4 | 2 | 2 | 3 | 1 | 1 | 1 |

**WARGEAR**

- Club
- knife (sword)

**SPECIAL RULES**

- **Let 'em have it!** `PASSIVE`

Rowan may declare Heroic Combats for free, but only if he will be able move into combat with an enemy Hobbit Hero model.

- **Put 'em in Chains** `ACTIVE`

Instead of making Strikes, Sid may choose a Hobbit model and roll a D6. If higher than the Hobbit's Strength, the Hobbit is bound in chains, treated as if it had suffered the Paralyse Magic Power.

- **Burn it Down!** `ACTIVE`

Once per game, if within 1" of a building or woodland terrain, roll a D6. On 3+, it is set on fire. Any model touching it must roll a D6. On 4+, they are Set Ablaze.


</div>

---

<div class="unitCard" markdown>

### RUFFIAN [5 POINTS]
*HERO OF FORTITUDE, 12 WARBAND*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 2/5+ | 3 | 3 | 1 | 1 | 2 |

**WARGEAR**

- Knife or club.

**OPTIONS**

- Swap Weapon: 5 points
- Bow: 1 point
- Whip: 1 point

**SPECIAL RULES**

- **At 'em Lads!** `ACTIVE`

A Ruffian that is Engaged in a Fight that involves one or more Hobbits gains +1 Fight value. Additionally, instead of making Strikes, a Ruffian may choose one Hobbit model and roll a D6. If it's higher than the Hobbit's Strength, the Hobbit is bound in chains, treated as if it had suffered the Paralyse Magic Power.


</div>

---

<div class="unitCard" markdown>

### TED SANDYMAN [20 POINTS]
*HERO OF FORTITUDE, UNIQUE, 12 WARBAND*

| Mv | F | S | D | A | W | C | M | W | F |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 4" | 2/3+ | 2 | 3 | 1 | 1 | 3 | 1 | 1 | 1 |

**WARGEAR**

- Hammer

**SPECIAL RULES**

- Resistant to Magic
- Throw Stones (range 8", Strength 1)

- **A Bad Influence** `PASSIVE`

Ted may include only include Hobbit models in his warband, and may include Hobbit Militia as if they were from Sharkey's Rogues army list.


</div>

---


## WARRIORS OF SHARKEY'S ROGUES

<div class="unitCard" markdown>

### HOBBIT MILITIA [4 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 4" | 1/3+ | 2 | 3 | 1 | 1 | 3 |

**WARGEAR**

- Dagger
- axe or hammer

**OPTIONS**

- Swap Weapon: 1 point

**SPECIAL RULES**

- Resistant to Magic
- Throw Stones (range 8", Strength 1)

</div>

---

<div class="unitCard" markdown>

### RUFFIAN [5 POINTS]
*WARRIOR*

| Mv | F | S | D | A | W | C |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 6" | 2/5+ | 3 | 3 | 1 | 1 | 2 |

**WARGEAR**

- Knife or club.

**OPTIONS**

- Bow: 1 point
- Whip: 1 point
- Swap Weapon: 1 point

**SPECIAL RULES**

- **At 'em Lads!** `ACTIVE`

A Ruffian that is Engaged in a Fight that involves one or more Hobbits gains +1 Fight value. Additionally, instead of making Strikes, a Ruffian may choose one Hobbit model and roll a D6. If it's higher than the Hobbit's Strength, the Hobbit is bound in chains, treated as if it had suffered the Paralyse Magic Power.


</div>
